	...
	for ut in  `seq $UTIL_START $UTIL_STEP $UTIL_END` ; do
		...
		while [ $nsuccess -lt $SSIZE ] && [ $ifile -lt $nfile ] ; do
			...
			if [ -e "$file.r.tight" ] ; then
				if [ "$(cat $file.r.tight | wc -l)" == "3" ] ; then
					if [ "$(head -1 $file.r.tight)" == "0" ] ; then
						udone="yes"
					fi
				else
					if [ $(cat $file.r.tight | wc -l) -gt 3 ] ; then
						udone="yes"
					fi
				fi
			fi
			if [ -z "$udone" ] ; then
				TEMPFS="$SIM_TIGHT $TIME_PAR -rstm $file"
				touch $file.r.tight
				$TEMPFS  > $file.r.tight
			fi
			usucc=$(head -1 $file.r.tight)

			edone=""
			# execute exact
			if [ -e "$file.r.exact" ] ; then
				if [ "$(cat $file.r.exact | wc -l)" == "3" ] ; then
					if [ "$(head -1 $file.r.exact)" == "0" ] ; then
						edone="yes"
					fi
				else
					if [ $(cat $file.r.exact | wc -l) -gt 3 ] ; then
						edone="yes"
					fi
				fi
			fi
			if [ -z "$edone" ] ; then
				TEMPFS="$SIM_EXACT $TIME_PAR -rstm $file"
				$TEMPFS > $file.r.exact
			fi
			esucc=$(head -1 $file.r.exact)

			edfdone=""
			# execute edf
			if [ -e "$file.r.edf" ] ; then
				if [ "$(cat $file.r.edf | wc -l)" == "3" ] ; then
					if [ "$(head -1 $file.r.edf)" == "0" ] ; then
						edfdone="yes"
					fi
				else
					if [ $(cat $file.r.edf | wc -l) -gt 3 ] ; then
						edfdone="yes"
					fi
				fi
			fi
			if [ -z "$edfdone" ] ; then
				TEMPFS="$SIM_EDF $TIME_PAR -rstm $file"
				$TEMPFS > $file.r.edf
			fi
			edfsucc=$(head -1 $file.r.edf)
