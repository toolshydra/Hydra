...
DEST=$SCHED_HOME/run/exercise_b_30
...
for NTASKS in  `seq -w $TASK_START $TASK_STEP $TASK_END` ; do
	for ut in  `seq $UTIL_START $UTIL_STEP $UTIL_END` ; do
		...
		while [ $ifile -lt $nfile ] ; do
			...
			file=${file%%.tight}
			ifile=$(($ifile + 1))
			# copy only files that have success on all solvers
			if [ "$(head -1 $file.tight)" == "1" ] &&
			   [ "$(head -1 $file.edf)" == "1" ] &&
			   [ "$(head -1 $file.exact)" == "1" ] ; then
			   cp $file $DEST/$ut/$NTASKS/
			fi
